...
public class JudgeMain extends javax.swing.JApplet implements Codes{
    ...
    JButton[] tests;
    /**
     * Initializes the applet JudgeMain
     */
    @Override
    public void init() {
        /*
    ...
    private void print(byte[] b, int len) {
        print(new String(b,0,len));
    }
    ...
    private void submit() {
        ...
        String name=divide[divide.length-2]+"."+divide[divide.length-1];
        boolean matchFound = false;
        for(int i=0;i<acceptableFileExtensions[language].length;i++) {
            if(extension.toLowerCase().contentEquals(acceptableFileExtensions[language][i].toLowerCase())) {
                matchFound = true;
                break;
            }
        }
